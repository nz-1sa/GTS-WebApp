...
function ascii8bitStrToPackedInts(str:string):number[] {
	let ints:number[] = new Array(Math.ceil(str.length/4));	// The array will be a quater of the length of the string, rounding up to make room for any dangling chars in the final block.
...
function packedIntsToAscii8bitStr(ints:number[]):string {
	let a:string[] = new Array(ints.length);		// Each output array entry will be a string consisting of four characters unpacked from each 32bit int
	for (var i=0; i<ints.length; i++) {
...
function padLeft(str:string):string{
	if(str.length>=2){ return str; } else { return padLeft("0"+str); }	// Recurse to provide "00" from empty string.
}

// For each character in a unicode string return its char code. Result is an int array of the same length as the string
function unicodeStrToInts(str:string):number[] {
...
function murmurHash3_x86_128(key:number[], seed:number = 0):number[] {
    ...
    var k1:number, k2:number, k3:number, k4:number, p1:number = 597399067, p2:number = 2869860233, p3:number = 951274213, p4:number = 2716044179;

    var h1:number = seed ^ p1,
        h2:number = seed ^ p2,
        h3:number = seed ^ p3,
        h4:number = seed ^ p4;

    for(var i:number = 0, b:number = key.length & -16; i < b;) {
...
export function encrypt(plaintext:string, password:string, seed:number = 0):string {
	if (plaintext === undefined || plaintext.length === 0) return (''); 	// Empty/missing string stays the same, nothing to encrypt.
	...
	let asciitext:string = encodeURI(plaintext).replace(/%20/g,' ');

	// Convert the text to encode to an array of ints.
	let v:number[] = ascii8bitStrToPackedInts(asciitext);
	...
	let k:number[] = murmurHash3(password, seed);

	// Do the encryption.
	let n:number = v.length;
	let z:number = v[n-1], y:number = v[0], delta:number = 0x9E3779B9;
	var mx:number, e:number, q:number = Math.floor(16 + 32/n), sum:number = 0;		// academic exploit fix; https://crypto.stackexchange.com/questions/12993/in-what-way-is-xxtea-really-vulnerable
	...
	let ciphertext:string = packedIntsToAscii8bitStr(v);
...
export function decrypt(ciphertexthex:string, password:string, seed:number = 0){
	if (ciphertexthex === undefined || ciphertexthex.length === 0) return (''); // Empty/missing string stays the same, nothing to decrypt.
	...
	let v:number[] = ascii8bitStrToPackedInts(ciphertext);
	...
	let n:number = v.length;
	let z:number = v[n-1], y:number = v[0], delta:number = 0x9E3779B9;
	var mx:number, e:number, q:number = Math.floor(16 + 32/n), sum:number = q*delta;				// academic exploit fix; https://crypto.stackexchange.com/questions/12993/in-what-way-is-xxtea-really-vulnerable
	while (sum != 0) {
		e = sum>>>2 & 3;
		for (var p = n-1; p >= 0; p--) {
			z = v[p>0 ? p-1 : n-1];
	...
	let plaintext:string = packedIntsToAscii8bitStr(v);
